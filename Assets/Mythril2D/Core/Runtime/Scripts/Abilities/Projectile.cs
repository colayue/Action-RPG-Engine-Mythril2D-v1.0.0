...
namespace Gyvr.Mythril2D
{
    ...
    public class Projectile : MonoBehaviour
    {
        ...
        private void OnCollisionEnter2D(Collision2D collision) => TryColliding(collision.gameObject);

        private void OnTriggerEnter2D(Collider2D collision)
        {
            if (collision.gameObject.layer == LayerMask.NameToLayer(GameManager.Config.hitboxLayer))
            {
